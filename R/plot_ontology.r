...
plot_enrichment <- function(
    ...
    pval = 0.01, fold_change = 1, signif = 0.01,
    top=10, hide_insig = TRUE, 
    colors = c("#50FA7B", "#44475A"), labeller = NA,
    ...
) {
    ...
    de_genes <- dplyr::mutate(de_genes, signif = dplyr::case_when(
    ...
    go_fisher_classic <- topGO::runTest(
        go_data, algorithm = 'classic', statistic = 'fisher'
    )
    go_fisher_elim <- topGO::runTest(
        go_data, algorithm = 'elim', statistic = 'fisher'
    )
    go_kruskal_classic <- topGO::runTest(
        go_data, algorithm = 'classic', statistic = 'KruskalWallis'
    )
    go_kruskal_elim <- topGO::runTest(
        go_data, algorithm = 'elim', statistic = 'KruskalWallis'
    )
