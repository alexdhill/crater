...
plot_expression <- function(
    ...
    if (class(gene_ids) == "logical") {
        if (length(gene_ids) != nrow(dds)) {
            stop(paste0(
                "If gene_ids is a logical vector, it must be the same length as the ",
                "number of rows in dds"
            ))
        }
    ...
    if (counts == "raw") {
        expr <- SummarizedExperiment::assays(dds)$counts[
            gene_ids,
            ,
            drop = FALSE
        ]
    ...
    if (counts == "normalized") {
        ...
        if (length(contrast) == 1) {
            ...
            signif <- apply(comps, 1, function(comp) {
                res <- DESeq2::results(
                    dds,
                    contrast = c(contrast[1], comp[1], comp[2])
                )
            ...
            signif <- dplyr::left_join(
                signif,
                expr[, c("gene_id", "count")],
                by = "gene_id",
                relationship = "many-to-many"
            )
            signif <- dplyr::summarize(
                signif,
                ymax = max(count),
                .by = colnames(signif)[colnames(signif) != "count"]
            )
            ...
            signif <- dplyr::mutate(
                signif,
                ypos = ymax * (1 + step * (dplyr::row_number()))
            )
            ...
            expr <- dplyr::filter(
                expr,
                .data[[contrast[1]]] %in% c(contrast[2], contrast[3])
            )
        ...
        signif <- dplyr::mutate(
            ...
            signif = ifelse(
                ...
                ifelse(
                    padj < 0.001,
                    "***",
                    ifelse(padj < 0.01, "**", ifelse(padj < 0.05, "*", "ns"))
                )
            )
        )
        signif <- dplyr::left_join(
            signif,
            as.data.frame(SummarizedExperiment::rowData(dds))[, c(
                "gene_id",
                "gene_name",
                color_by
            )],
        ...
        signif <- dplyr::mutate(
            signif,
            xstart = as.numeric(numer),
            xend = as.numeric(denom)
        )
    } else {
    ...
    if (counts == "normalized") {
        ...
        expr_plot <- expr_plot +
            ...
            ggplot2::scale_y_continuous(
                expand = ggplot2::expansion(mult = c(step, step))
            )
    }
