...
plot_enrichment <- function(
    ...
    if (hide_insig) {
        enrichment <- dplyr::filter(enrichment, padj < signif_level)
    }
    enrichment <- dplyr::mutate(
        enrichment,
        signif = ifelse(padj < signif_level, TRUE, FALSE)
    )
    enrichment <- dplyr::mutate(
        enrichment,
    ...
    plot <- ggplot2::ggplot(
        enrichment,
        ggplot2::aes(x = reorder(pathway, NES), y = NES, color = signif)
    ) +
    ...
    if (!is.na(labeller)) {
        plot <- plot + ggplot2::scale_x_discrete(labels = labeller)
    }
