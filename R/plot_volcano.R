...
plot_volcano <- function(
    ...
    res <- dplyr::mutate(
        ...
        signif = dplyr::case_when(
            (padj < p_threshold) &
                (abs(log2FoldChange) >= lfc_threshold) ~ "Significant",
            TRUE ~ "Not Significant"
        )
    )
    res <- dplyr::mutate(
        res,
        tmp_color_signif = dplyr::case_when(
    ...
    res <- dplyr::mutate(
        res,
        tmp_color_signif = factor(
            tmp_color_signif,
            levels = c(names(colors), "Not Significant")
        )
    )

    missing_levels <- setdiff(
        levels(res$tmp_color_signif),
        levels(factor(res$tmp_color_signif))
    )
    ...
    volcano_plot <- ggplot2::ggplot(
        res,
        ggplot2::aes(x = as.numeric(log2FoldChange), y = as.numeric(negLogPadj))
    ) +
        ggplot2::geom_hline(
        ...
        ggplot2::scale_color_manual(
            ...
            breaks = names(colors),
            limits = levels(res$tmp_color_signif)
        ) +
