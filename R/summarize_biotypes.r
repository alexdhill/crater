...
summarize_biotypes <- function(data, biotypes="gene_biotype", gene_ids='gene_id', gene_names="gene_name", biotypes_to=NA) {
    ...
    if (gene_ids %in% colnames(data)) summarized_biotypes$gi <- data[[gene_ids]]
    else stop("Need gene_ids to summarize biotypes")
    if (gene_names %in% colnames(data)) summarized_biotypes$gn <- data[[gene_names]]
    else stop("Need gene_names to summarize biotypes")
